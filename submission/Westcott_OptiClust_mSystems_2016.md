...
Amplicon-based sequencing has provided incredible insights into Earth's microbial biodiversity. It has become common for studies to include sequencing millions of 16S rRNA gene sequences across hundreds of samples. This is three to four orders of magnitude greater sequencing depth than was previously achieved using Sanger sequencing. The increased sequencing depth has highlighted the limitations of existing bioinformatics tools for analyzing these sequences. Furthermore, these sequencing efforts have revealed novel taxonomic diversity that are not adequately represented in reference databases. Because of this, a critical component to the analysis of microbial biodiversity has been the assignment of 16S rRNA gene sequences and other marker gene sequences to operational taxonomic units (OTUs) without reliance on taxonomic classification.

A growing number of algorithms have been developed to assign sequences into OTUs and these can be clustered into three general categories. The first category of algorithms are classification-based and have been termed closed-reference or phylotyping. Sequences are compared to a reference collection and clustered based on the reference sequence that they are similar to. This approach is fast; however, struggles when a sequence is similar to multiple reference sequences that may have different taxonomies. The second category of algorithm have been called *de novo* because they assign sequences to OTUs without the use of a reference. These include hierarchical algorithms such as nearest, furthest, and average neighbor and methods that employ heuristics such as abundance or distance-based greedy clustering as implemented in USEARCH or VSEARCH, Sumaclust, OTUCLUST, and Swarm. These methods tend to be more computationally intense and it has proven difficult to know which method generates the best assignments. A third category of algorithm is a hybrid approach or open-reference clustering. Here sequences are assigned to OTUs using closed-reference clustering and sequences that are not within a threshold of a reference sequence are then clustered using a *de novo* approach. This category blends the strengths and weaknesses of the other method and adds the complication that closed-reference and *de novo* clustering use different OTU definitions.

Several metrics have emerged for assessing the quality of OTU assignment algorithms. These have included the time and memory required to run the algorithm, agreement between OTU assignments and the sequences' taxonomy, sensitivity of an algorithm to stochastic processes, and the number of OTUs generated by the algorithm. Unfortunately, these methods fail to directly quantify the quality of the OTU assignments. An algorithm may complete with minimal time and memory requirements or generate an idealized number of OTUs, but the composition of the OTUs could be incorrect. These metrics also tend to be subjective. For instance, a method may appear to be recapitulate the taxonomy of a synthetic community with known taxonomic structure, but do a poor job when applied to real communities with poorly defined taxonomic structure or for sequences that are prone to misclassification. As an alternative, we developed an approach to objectively benchmark the clustering quality of OTU assignments. This approach counts the number of true positives (TP), true negatives (TN), false positives (FP), and false negatives (FN) based on the pairwise distances. Sequence pairs that are within the user-specified threshold and are clustered together represent TPs and those in different OTUs are FNs. Those sequence pairs that have a distance larger than the threshold and are not clustered in the same OTU are TNs and those in the same OTU are FPs. These values can be synthesized into a single correlation coefficient, the Matthews' Correlation Coefficient (MCC), which measures the correlation between observed and predicted classifications and is robust to cases where there is an uneven distribution across the confusion matrix.

We previously used this approach to assess the OTU assignments of a variety of algorithms using synthetic and real datasets. Consistently, the average neighbor algorithm was identified as among the best or the best algorithm. The distance-based greedy clustering as implemented in VSEARCH has also performed well. The computational resources required to complete the average neighbor algorithm can be significant for large datasets and so there is a need for an algorithm that efficiently produces consistently high quality OTU assignments. Previously, we assessed the quality of the clusters after the completion of the algorithm or post hoc. In the current study we developed and benchmarked a new de novo clustering algorithm. Instead of assessing the methods post hoc, the new method uses a priori calculation of the MCC to direct the algorithm. The result is the OptiClust algorithm, which produces significantly better sequence assignments while making efficient use of computational resources.
...
As implemented within mothur, the OptiClust algorithm either starts with each sequence in a separate OTU or with all of the sequences in a single OTU. We repeated the complete convergence analysis, but seeded the algorithm with all sequences in a single OTU. We found that the MCC values for clusters generated seeding OptiClust with the sequences as a single OTU were between 0.1 and 11.5% lower than when seeding the algorithm with sequences in separate OTUs (Figure 1). Interestingly, with the exception of the 2 dataset (0.2% more OTUs), the number of OTUs was as much as 7.0% lower (4) than when the algorithm was seeded with sequence in separate OTUs. Finally, the amount of time required to cluster the data when the algorithm was seeded with a single OTU was between 1.6 and 2.6-times longer than if sequences were seeded as separate OTUs. This analysis demonstrates that seeding the algorithm with sequences as separate OTUs results in the best OTU assignments in the shortest amount of time.
