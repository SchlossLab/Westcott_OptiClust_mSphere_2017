...
The rapid expansion in sequencing capacity has demonstrated that database-independent assignment of sequences into OTUs is a critical step in the sequence analysis pipeline. Of course, the quality of the assignments is dependent on the quality of the underlying data. The ability to objectively optimize the clustering quality will significantly enhance the quality of those OTU assignments while making the time and memory requirements more efficient. The development of the OptiClust algorithm represents a significant advance that likely has numerous other applications.
